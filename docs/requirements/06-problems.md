...
**Open status** below reflects the official roadmap at capture (18 Sep 2026) and may change as organisers release problems.
...
| `PR-04` | The New Patient | `the_new_patient` | 4 | 2 | not yet |
| `PR-05` | When Exactly | `when_exactly` | 5 | 2 | not yet |
| `PR-06` | The Rules | `the_rules` | 5 | 3 | not yet |
...
Max board from full roster: **49** (`SC-max`). Per-case walkthroughs deferred until client examples are shared.
...
| **Must handle** | Two surnames, DNI/NIE + check letter, DOB, phone, email, insurer. Caller declines appointment if offered. `BOOK` alongside registration fails. Sharp ASR test; email has no check digit. |
| **Links** | `FR-register`, `CR-register`, `SC-norm-*` |
...
| **Traps** | Sur Friday lunchtime shut; only Centro Saturday; nothing Sunday; network shut Mon 12 Oct Fiesta. If requested day closed, caller takes earliest on next open day still matching rest of ask. |
| **Links** | `FR-relative-time`, `CL-fiesta`, `CL-clock` |
...
| **Links** | `FR-blocked`, `FR-clinic-rules`, `CR-reason-closed` |
...
Open problems (1–3) ingested from the team dashboard extract (18 Sep 2026): see [scenarios/](scenarios/README.md). Remaining problems (`PR-04`…`PR-18`) still await examples when they open.
