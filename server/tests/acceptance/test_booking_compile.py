...
def slot(provider, location, kind, start):
    return {
        ...
        "start_time": start,
        "duration_minutes": 15,
        "payable_with": ["mapfre", "sanitas", "cigna"],
    }
...
def test_josefa_skips_same_day():
    av = availability(
        slot("PR01", "centro", "review", "2026-09-18T11:00:00+02:00"),
...
def test_amelia_first_visit():
    av = availability(
        slot("PR07", "centro", "first_visit", "2026-09-18T12:00:00+02:00"),
...
def test_ignacio_orthopaedic():
    av = availability(
        slot("PR10", "sur", "orthopaedic_review", "2026-09-18T09:00:00+02:00"),
...
def test_chloe_monday_morning():
    av = availability(
        slot("PR03", "sur", "review", "2026-09-18T09:00:00+02:00"),  # same day
    ...
    assert pick_offer(
        av, patient("P00011", "mapfre", True), CONNECTED, weekday="monday", part_of_day="morning"
    ) == {
        "patient_id": "P00011",
        "provider_id": "PR03",
        "location_id": "sur",
