...
def test_offer_after_a_refusal_clears_the_stale_reason():
    # blocked -> caller changes specialty -> a slot is offered -> the call is cut off.
    client = FlakyClient(failures=0)
    ...
    assert client.posted[0] == {"call_id": "c3", "action": "NO_ACTION", "reason": NO_OUTCOME_REASON}
