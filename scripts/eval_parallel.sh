#!/usr/bin/env bash
# Runs server/evals/*.yaml across N concurrent bot instances on separate ports,
...
while IFS= read -r line; do
	FILES+=("$line")
done < <(cd "$ROOT/server" && ls evals/*.yaml 2>/dev/null | grep -v '/spec/')
if [ "${#FILES[@]}" -eq 0 ]; then
	echo "No scenario files found under server/evals/*.yaml" >&2
	exit 1
fi
